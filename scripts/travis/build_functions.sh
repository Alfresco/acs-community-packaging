...
function buildUpstreamTag() {
  ...
  mvn -B -V -ntp clean package -DskipTests -Dmaven.javadoc.skip=true "-Dimage.tag=${TAG}" ${EXTRA_BUILD_ARGUMENTS}
...
function buildSameBranchOnUpstream() {
  ...
  mvn -B -V -q -ntp clean install -DskipTests -Dmaven.javadoc.skip=true ${EXTRA_BUILD_ARGUMENTS}
  mvn -B -V -q -ntp install -DskipTests -f packaging/tests/pom.xml
