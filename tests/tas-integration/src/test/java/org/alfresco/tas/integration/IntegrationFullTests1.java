...
public class IntegrationFullTests1 extends IntegrationTest
{
     ...
     * 3. U1 gets site containers for site1, only documentLibrary is returned site with RestAPI
     * 4. U1 gets site container documentLibrary, proper details are returned using RestAPI
     * 5. U1 creates new container 'siteContainer1' using JS API
     * 6. U1 gets site containers for site1: documentLibrary and 'siteContainer1' are returned using RestAPI
     * 7. U1 gets site container 'siteContainer1', proper details are returned using RestAPI
     * 8. U1 creates new site container 'siteContainer2' using JS API
     * 9. U1 gets site containers for site1: documentLibrary, siteContainer2 and siteContainer1 are returned using RestAPI
     * 10. U1 gets site container 'siteContainer2' proper details are returned RestAPI
     */
    @Test(groups = { TestGroup.INTEGRATION, TestGroup.REQUIRE_JMX, TestGroup.FULL, TestGroup.EXTENTION_POINTS  })
    @TestRail(section = { TestGroup.INTEGRATION, TestGroup.SITES }, executionType = ExecutionType.REGRESSION,
            description = "Verify user can create custome site container with JS APi and get is with RestAPI")
    public void getCustomSiteContainers() throws Exception
    {
        dataContent.assertExtensionAmpExists("alfresco-contentmodel-extension");
        ...
        STEP("3. U1 gets site containers for site1, only documentLibrary is returned site with RestAPI");
        RestSiteContainerModelsCollection containers = restAPI.authenticateUser(testUser1).withCoreAPI().usingSite(testSitePublic).getSiteContainers();
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        containers.assertThat().paginationField("count").is("1");
        containers.assertThat().entriesListContains("folderId", "documentLibrary");
        
        STEP("4. U1 gets site container documentLibrary, proper details are returned using RestAPI");
        RestSiteContainerModel docLibContainer = restAPI.withCoreAPI().usingSite(testSitePublic).getSiteContainer("documentLibrary");
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        docLibContainer.assertThat().field("id").isNotNull()
            .and().field("folderId").is("documentLibrary");
        
        STEP("5. U1 creates new container 'siteContainer1' using JS API");
        String customContainer1 = "documentLibraryCustom";
        String containerUrl= "alfresco/s/tas/content";
        Map<String, String> paramsSendEmailActionUrl = new HashMap<String, String>();
        paramsSendEmailActionUrl.put("siteId", testSitePublic.getId());
        paramsSendEmailActionUrl.put("containerId", customContainer1);
        containerUrl = Utility.toUrlParams(containerUrl, paramsSendEmailActionUrl);
        
        RestAssured.basePath = "";
        restAPI.configureRequestSpec().setBasePath(RestAssured.basePath);
        RestRequest request =  RestRequest.simpleRequest(HttpMethod.GET, containerUrl);  
        restAPI.authenticateUser(dataUser.getAdminUser());
        RestResponse response = restAPI.process(request);
        response.getResponse().asInputStream();
        Assert.assertEquals(response.getResponse().getStatusCode(), HttpStatus.OK.value());
        
        STEP("6. U1 gets site containers for site1: documentLibrary and 'siteContainer1' are returned using RestAPI");
        containers = restAPI.withCoreAPI().usingSite(testSitePublic).getSiteContainers();
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        containers.assertThat().paginationField("count").is("2");
        containers.assertThat().entriesListContains("folderId", "documentLibrary")
            .and().entriesListContains("folderId", customContainer1);
        
        STEP("7. U1 gets site container 'siteContainer1', proper details are returned using RestAPI");
        docLibContainer = restAPI.withCoreAPI().usingSite(testSitePublic).getSiteContainer(customContainer1);
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        docLibContainer.assertThat().field("id").isNotNull()
            .and().field("folderId").is(customContainer1);
        
        STEP("8. U1 creates new site container 'siteContainer2' using JS API");
        String containerUrl2= "alfresco/s/tas/content";
        String customContainer2 = "customDocLib2";
        Map<String, String> paramsSendEmailActionUrl2 = new HashMap<String, String>();
        paramsSendEmailActionUrl2.put("siteId", testSitePublic.getId());
        paramsSendEmailActionUrl2.put("containerId", customContainer2);
        containerUrl2 = Utility.toUrlParams(containerUrl2, paramsSendEmailActionUrl2);
        
        RestAssured.basePath = "";
        restAPI.configureRequestSpec().setBasePath(RestAssured.basePath);
        request =  RestRequest.simpleRequest(HttpMethod.GET, containerUrl2);  
        restAPI.authenticateUser(dataUser.getAdminUser());
        response = restAPI.process(request);
        response.getResponse().asInputStream();
        Assert.assertEquals(response.getResponse().getStatusCode(), HttpStatus.OK.value());
        
        STEP("9. U1 gets site containers for site1: documentLibrary, siteContainer2 and siteContainer1 are returned using RestAPI");
        containers = restAPI.withCoreAPI().usingSite(testSitePublic).getSiteContainers();
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        containers.assertThat().paginationField("count").is("3");
        containers.assertThat().entriesListContains("folderId", "documentLibrary")
            .and().entriesListContains("folderId", customContainer1)
            .and().entriesListContains("folderId", customContainer2);
        
        STEP("10. U1 gets site container 'siteContainer2' proper details are returned RestAPI");
        docLibContainer = restAPI.withCoreAPI().usingSite(testSitePublic).getSiteContainer(customContainer2);
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        docLibContainer.assertThat().field("id").isNotNull()
            .and().field("folderId").is(customContainer2);
    }
