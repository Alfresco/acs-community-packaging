...
public class IntegrationFullTests extends IntegrationTest
{
    ...
    /**
     * Scenario
     * 1. Using CMIS create user testUser
     * 2. Using CMIS create public IMAP site testSite
     * 3. Using CMIS create folder testFolder in testSite document library
     * 4. Using CMIS add e-mail alias to testFolder
     * 5. Using SMTP send e-mail to testFolder alias address with a Rich Text Format file attached
     * 6. Using IMAP verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment
     * 7. Using CMIS verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment
     */
    @Test(groups = { TestGroup.INTEGRATION, TestGroup.REQUIRE_JMX, TestGroup.FULL  })
    @TestRail(section = { TestGroup.INTEGRATION, TestGroup.IMAP }, executionType = ExecutionType.REGRESSION,
            description = "Verify a message with Rich Text Format format from IMAP site is displayed correctly in IMAP client and in repository.")
    public void verifyMessageWithRTFAttachmentIsDisplayedCorrectlyInIMAPAndRepository() throws Exception
    {
        STEP("1. Using CMIS create user testUser");
        UserModel testUser = dataUser.createRandomTestUser();

        STEP("2. Using CMIS create public IMAP site testSite");
        SiteModel testSite = dataSite.usingUser(testUser).createIMAPSite();

        STEP("3. Using CMIS create folder testFolder in testSite document library");
        FolderModel testFolder = dataContent.usingUser(testUser).usingSite(testSite).createFolder();

        STEP("4. Using CMIS add e-mail alias to testFolder");
        String alias = dataContent.usingUser(testUser).usingResource(testFolder).addEmailAlias(RandomData.getRandomAlphanumeric());

        STEP("5. Using SMTP send e-mail to testFolder alias address with a Rich Text Format file attached");
        ...
        smtpProtocol.authenticateUser(testUser)
                ...
                .withSubject("subject")
                .withBody("test")
                .withAttachments(Utility.getResourceTestDataFile("test.rtf"))
                .sendMail();

        STEP("6. Using IMAP verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment");
        imapProtocol.authenticateUser(testUser).usingSite(testSite).usingResource(testFolder)
                .assertThat().containsMessages(new FileModel("test.rtf"), new FileModel("subject"));

        STEP("7. Using CMIS verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment");
        cmisAPI.authenticateUser(testUser).usingSite(testSite).usingResource(testFolder)
                .assertThat().hasChildren(new FileModel("test.rtf"), new FileModel("subject"));
    }
     ...
     * 3. Using CMIS create folder testFolder in testSite document library
     * 4. Using CMIS add e-mail alias to testFolder
     * 5. Using SMTP send e-mail to testFolder alias address with a HTML file attached
     * 6. Using IMAP verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment
     * 7. Using CMIS verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment
     */
    @Test(groups = { TestGroup.INTEGRATION, TestGroup.REQUIRE_JMX, TestGroup.FULL  })
    @TestRail(section = { TestGroup.INTEGRATION, TestGroup.IMAP }, executionType = ExecutionType.REGRESSION,
            description = "Verify a message with HTML format from IMAP site is displayed correctly in IMAP client and in repository")
    public void verifyMessageWithHTMLAttachmentIsDisplayedCorrectlyInIMAPAndRepository() throws Exception
    {
        STEP("1. Using CMIS create user testUser");
        UserModel testUser = dataUser.createRandomTestUser();
        ...
        STEP("5. Using SMTP send e-mail to testFolder alias address with a HTML file attached");
        dataGroup.usingUser(testUser).addUserToGroup(GroupModel.getEmailContributorsGroup());
        smtpProtocol.withJMX().updateSmtpUnknownUser(testUser.getUsername());
        smtpProtocol.authenticateUser(testUser)
                .and()
                ...
                .withSubject("subject")
                .withBody("test")
                .withAttachments(Utility.getResourceTestDataFile("test.html"))
                .sendMail();

        STEP("6. Using IMAP verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment");
        imapProtocol.authenticateUser(testUser).usingSite(testSite).usingResource(testFolder)
                .assertThat().containsMessages(new FileModel("test.html"), new FileModel("subject"));

        STEP("7. Using CMIS verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment");
        cmisAPI.authenticateUser(testUser).usingSite(testSite).usingResource(testFolder)
                .assertThat().hasChildren(new FileModel("test.html"), new FileModel("subject"));
    }
     ...
     * 3. Using CMIS create folder testFolder in testSite document library
     * 4. Using CMIS add e-mail alias to testFolder
     * 5. Using SMTP send e-mail to testFolder alias address with a msg file attached
     * 6. Using IMAP verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment
     * 7. Using CMIS verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment
     */
    @Test(groups = { TestGroup.INTEGRATION, TestGroup.REQUIRE_JMX, TestGroup.FULL  })
    @TestRail(section = { TestGroup.INTEGRATION, TestGroup.IMAP }, executionType = ExecutionType.REGRESSION,
            description = "Verify a message with attached .msg file from IMAP site is displayed correctly in IMAP client and in repository")
    public void verifyMessageWithMsgAttachmentIsDisplayedCorrectlyInIMAPAndRepository() throws Exception
    {
        STEP("1. Using CMIS create user testUser");
        UserModel testUser = dataUser.createRandomTestUser();
        ...
        STEP("5. Using SMTP send e-mail to testFolder alias address with a msg file attached");
        dataGroup.usingUser(testUser).addUserToGroup(GroupModel.getEmailContributorsGroup());
        smtpProtocol.withJMX().updateSmtpUnknownUser(testUser.getUsername());
        smtpProtocol.authenticateUser(testUser)
                .and()
                ...
                .withSubject("subject")
                .withBody("test")
                .withAttachments(Utility.getResourceTestDataFile("test.msg"))
                .sendMail();

        STEP("6. Using IMAP verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment");
        imapProtocol.authenticateUser(testUser).usingSite(testSite).usingResource(testFolder)
                .assertThat().containsMessages(new FileModel("test.msg"), new FileModel("subject"));

        STEP("7. Using CMIS verify that testFolder contains 2 files, a file named as the e-mail subject which content is the e-mail body and the attachment");
        cmisAPI.authenticateUser(testUser).usingSite(testSite).usingResource(testFolder)
                .assertThat().hasChildren(new FileModel("test.msg"), new FileModel("subject"));
    }
     ...
     * 2. U1 creates a public site using CMIS
     * 3. U1 creates a folder with a document (doc1.txt) in the public site using SMTP
     * 4. U1 renames document1 (doc1-edited.txt) using FTP
     * 5. U1 tries to copy doc1.txt to another location using CMIS
     * 6. Verify that document is not copied (catch Exceptions)
     */
    @Test(groups = { TestGroup.INTEGRATION, TestGroup.REQUIRE_JMX, TestGroup.FULL  }, expectedExceptions = CmisObjectNotFoundException.class)
    @TestRail(section = { TestGroup.INTEGRATION, TestGroup.CONTENT }, executionType = ExecutionType.REGRESSION,
            description = "Verify user is not able to copy a renamed file using its initial name.")
    public void copyRenamedFileUsingInitialFilename() throws Exception
    {
        ...
        STEP("3. U1 creates a folder with a document (doc1.txt) in the public site using SMTP");
        parentFolder = dataContent.usingUser(testUser1).usingSite(publicSite).createFolder();
        String alias = dataContent.usingResource(parentFolder).addEmailAlias(RandomData.getRandomAlphanumeric());
        ...
        smtpProtocol.authenticateUser(testUser1).usingResource(parentFolder)
                .and()
                .composeMessage()
                .withRecipients(alias + "@tas-alfresco.com")
                ...
                .sendMail();
        testFile1.setCmisLocation(smtpProtocol.getLastResource());

        STEP("4. U1 renames document1 (doc1-edited.txt) using FTP");
        FileModel oldFile = new FileModel(testFile1);
        ftpProtocol.authenticateUser(testUser1)
                .usingResource(testFile1).rename("new" + testFile1.getName())
                    .assertThat().existsInRepo();

        STEP("5. U1 tries to copy doc1.txt to another location using CMIS");
        FolderModel destination = new FolderModel(Utility.buildPath("Sites", publicSite.getId(), "documentLibrary"));
        cmisAPI.authenticateUser(testUser1).usingResource(oldFile).copyTo(destination);
    }
...
//               .assertThat().entriesListContains("processDefinitionId", WorkflowService.WorkflowType.NewTask.getId())
    ...
    /**
     * Scenario 99
     * 1. Using CMIS create user3 with no tenant.
     * 2. Using RestAPI create two tenants.
     * 3. Using RestAPI create two users: user1 for tenant1, user2 for tenant2.
     * 4. Using RestAPI verify all the existing networks for user1.
     * 5. Using RestAPI verify all the existing networks for user2.
     * 6. Using RestAPI verify all the existing networks for user3.
     */
    @Test(groups = { TestGroup.INTEGRATION, TestGroup.FULL, TestGroup.NETWORKS })
    @TestRail(section = { TestGroup.INTEGRATION, TestGroup.NETWORKS}, executionType = ExecutionType.REGRESSION,
            description = "Verify that tenant and non-tenant user can get information about networks.")
    public void getUserNetwork() throws Exception
    {
        STEP("1. Using CMIS create user3 with no tenant.");
        UserModel nonTenantUser3 = dataUser.usingAdmin().createRandomTestUser();

        STEP("2. Using RestAPI create two tenants.");
        UserModel adminTenant1 = UserModel.getAdminTenantUser();
        restAPI.authenticateUser(admin).usingTenant().createTenant(adminTenant1);

        UserModel adminTenant2 = UserModel.getAdminTenantUser();
        restAPI.usingTenant().createTenant(adminTenant2);

        STEP("3. Using RestAPI create two users: user1 for tenant1, user2 for tenant2.");
        ...
        STEP("4. Using RestAPI verify all the existing networks for user1.");
        restAPI.authenticateUser(tenantUser1);
        RestNetworkModelsCollection networks1 = restAPI.withCoreAPI().usingAuthUser().getNetworks();
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        networks1.getOneRandomEntry().onModel().assertNetworkIsEnabled().and().field("id").is(tenantUser1.getDomain().toLowerCase());

        restAPI.withCoreAPI().usingAuthUser().getNetwork();
        restAPI.assertStatusCodeIs(HttpStatus.OK);

        restAPI.withCoreAPI().usingNetworks().getNetwork(tenantUser1);
        restAPI.assertStatusCodeIs(HttpStatus.OK);

        STEP("5. Using RestAPI verify all the existing networks for user2.");
        restAPI.authenticateUser(tenantUser2);
        RestNetworkModelsCollection networks2 = restAPI.withCoreAPI().usingAuthUser().getNetworks();
        restAPI.assertStatusCodeIs(HttpStatus.OK);
        networks2.getOneRandomEntry().onModel().assertNetworkIsEnabled().and().field("id").is(tenantUser2.getDomain().toLowerCase());

        restAPI.withCoreAPI().usingAuthUser().getNetwork();
        restAPI.assertStatusCodeIs(HttpStatus.OK);

        restAPI.withCoreAPI().usingAuthUser().getNetwork(tenantUser1);
        restAPI.assertStatusCodeIs(HttpStatus.NOT_FOUND);

        restAPI.withCoreAPI().usingNetworks().getNetwork(tenantUser2);
        restAPI.assertStatusCodeIs(HttpStatus.OK);

        restAPI.withCoreAPI().usingNetworks().getNetwork(tenantUser1);
        restAPI.assertStatusCodeIs(HttpStatus.FORBIDDEN);

        STEP("6. Using RestAPI verify all the existing networks for user3.");
        restAPI.authenticateUser(nonTenantUser3);

        restAPI.withCoreAPI().usingAuthUser().getNetwork(tenantUser1);
        restAPI.assertStatusCodeIs(HttpStatus.NOT_FOUND);

        restAPI.withCoreAPI().usingAuthUser().getNetwork(tenantUser2);
        restAPI.assertStatusCodeIs(HttpStatus.NOT_FOUND);
    ...
    /**
     * Scenario
     *  1. Get alfresco-pdf-renderer version and availability using Jmx
     *  2. Verify that the availability attribute is "true" by default
     *  3. Verify that the version is displayed as "Alfresco PDF Renderer Version * using Pdfium *"
     */
	@Test(groups = { TestGroup.INTEGRATION, TestGroup.REQUIRE_JMX, TestGroup.FULL })
	@TestRail(section = { TestGroup.INTEGRATION,
			TestGroup.CONTENT }, executionType = ExecutionType.REGRESSION, description = "Verify that JMX properties for alfresco-pdf-renderer display version and availability")
	public void readAlfrescoPdfRendererJmxProperties() throws Exception
	{
		STEP("1. Verify alfresco-pdf-renderer availability using Jmx");
		String status = dataUser.withJMX()
				.getPropertyValue("Alfresco:Name=ContentTransformer,Type=Alfresco-PDF-Renderer", "Available");
		Assert.assertTrue(status.equals("true"), "Alfresco-Pdf-Renderer should be available");
		
		STEP("2. Verify that the version is displayed as 'Alfresco PDF Renderer Version * using Pdfium *'");
		String version = dataUser.withJMX()
				.getPropertyValue("Alfresco:Name=ContentTransformer,Type=Alfresco-PDF-Renderer", "VersionString");
		Assert.assertTrue(version.contains("Alfresco PDF Renderer Version") && (version.contains("using Pdfium")),
				"Alfresco-Pdf-Renderer version is not displayed properly");
	}
}
