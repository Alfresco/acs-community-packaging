...
public class IntegrationSanityTests extends IntegrationTest
{
    ...
    /**
     * Scenario 8
     * 1. Using CMIS, create 1 test user
     * 2. Using CMIS, create a public test site
     * 3. Using CMIS, WebDAV, FTP and SMTP, create a document
     * 4. Using CMIS, check out the documents
     * 5. Using CMIS, WebDAV and FTP, verify if PWC is created
     * 6. Using CMIS, WebDAV and FTP, delete PWC
     * 7. Using CMIS, verify if the PWC has been deleted
     * 8. Using CMIS, verify original document has version 1.0
     */
    @Test(groups = { TestGroup.INTEGRATION, TestGroup.REQUIRE_JMX, TestGroup.SANITY  })
    @TestRail(section = { TestGroup.INTEGRATION, TestGroup.CONTENT }, executionType = ExecutionType.SANITY,
            description = "Verify user with manager role can check out document")
    public void checkOutDocumentByUserWithManagerRole() throws Exception
    {
        STEP("Step 1: Using CMIS, create 1 test user");
        testUser1 = dataUser.createRandomTestUser();

        STEP("Step 2: Using CMIS, create a public test site");
        testSitePublic = dataSite.usingUser(testUser1).createIMAPSite();

        testFolder1 = dataContent.usingUser(testUser1).usingSite(testSitePublic).createFolder();
        String alias = dataContent.usingResource(testFolder1).addEmailAlias(RandomData.getRandomAlphanumeric());
        FileModel cmisPWCFile, webdavPWCFile, ftpPWCFile, smtpPWCFile;
        ...
        FileModel ftpTestFile = FileModel.getRandomFileModel(FileType.TEXT_PLAIN, "file 4 content");
        FileModel smtpTestFile = FileModel.getRandomFileModel(FileType.TEXT_PLAIN, "file 5 content");

        STEP("Step 3: Using CMIS, WebDAV, FTP and SMTP, create a document");
        cmisAPI.authenticateUser(testUser1).usingResource(testFolder1).createFile(cmisTestFile).assertThat().existsInRepo();
        webDavProtocol.authenticateUser(testUser1).usingResource(testFolder1).createFile(webdavTestFile).assertThat().existsInWebdav();
        ftpProtocol.authenticateUser(testUser1).usingResource(testFolder1).createFile(ftpTestFile).assertThat().existsInFtp();
        ...
        smtpProtocol.usingResource(testFolder1).authenticateUser(testUser1)
                .and()
                .composeMessage()
                ...
                .sendMail();
        smtpTestFile.setCmisLocation(smtpProtocol.getLastResource());
        imapProtocol.authenticateUser(testUser1).usingResource(testFolder1).assertThat().containsMessages(smtpTestFile);

        STEP("Step 4: Using CMIS, check out the documents");
        cmisAPI.usingResource(cmisTestFile).checkOut();
        cmisAPI.usingResource(webdavTestFile).checkOut();
        cmisAPI.usingResource(ftpTestFile).checkOut();
        cmisAPI.usingResource(smtpTestFile).checkOut();

        STEP("Step 5: Using CMIS, WebDAV and FTP, verify if PWC is created");
        cmisPWCFile = cmisAPI.usingResource(cmisTestFile).withCMISUtil().getPWCFileModel();
        webdavPWCFile = cmisAPI.usingResource(webdavTestFile).withCMISUtil().getPWCFileModel();
        ftpPWCFile = cmisAPI.usingResource(ftpTestFile).withCMISUtil().getPWCFileModel();
        smtpPWCFile = cmisAPI.usingResource(smtpTestFile).withCMISUtil().getPWCFileModel();

        cmisAPI.usingResource(cmisPWCFile).assertThat().existsInRepo();
        webDavProtocol.usingResource(webdavPWCFile).assertThat().existsInWebdav();
        ftpProtocol.usingResource(ftpPWCFile).assertThat().existsInRepo();
        cmisAPI.usingResource(smtpPWCFile).assertThat().existsInRepo();

        STEP("Step 6: Using CMIS, WebDAV and FTP, delete PWC");
        cmisAPI.usingResource(cmisPWCFile).delete();
        webDavProtocol.usingResource(webdavPWCFile).delete();
        ftpProtocol.usingResource(ftpPWCFile).delete();
        cmisAPI.usingResource(smtpPWCFile).delete();

        STEP("Step 7-8: Using CMIS, verify if the PWC has been deleted and original document has version 1.0");
        cmisAPI.usingResource(cmisPWCFile).assertThat().doesNotExistInRepo()
                .then().usingResource(cmisTestFile).assertThat().documentHasVersion(1.0);
        cmisAPI.usingResource(webdavPWCFile).assertThat().doesNotExistInRepo()
                .then().usingResource(webdavTestFile).assertThat().documentHasVersion(1.0);
        cmisAPI.usingResource(ftpPWCFile).assertThat().doesNotExistInRepo()
                .then().usingResource(ftpTestFile).assertThat().documentHasVersion(1.0);
        cmisAPI.usingResource(smtpPWCFile).assertThat().doesNotExistInRepo()
                .then().usingResource(smtpTestFile).assertThat().documentHasVersion(1.0);
    }
     ...
     * 4. Using CMIS, check out the documents
     * 5. Using CMIS, WebDAV and FTP, verify if PWC is created
     * 6. Using CMIS, cancel check out
     * 7. Using CMIS, verify original document has version 1.0
     */
    @Test(groups = { TestGroup.INTEGRATION, TestGroup.REQUIRE_JMX, TestGroup.SANITY  })
    @TestRail(section = { TestGroup.INTEGRATION, TestGroup.CONTENT }, executionType = ExecutionType.SANITY,
            description = "Verify user with manager role can check out document")
    public void cancelCheckOutDocumentByUserWithManagerRole() throws Exception
    {
        STEP("Step 1: Using CMIS, create 1 test user");
        testUser1 = dataUser.createRandomTestUser();
        ...
        STEP("Step 6: Using CMIS, cancel check out");
        cmisAPI.usingResource(cmisTestFile).cancelCheckOut();
        cmisAPI.usingResource(webdavTestFile).cancelCheckOut();
        cmisAPI.usingResource(ftpTestFile).cancelCheckOut();
        cmisAPI.usingResource(smtpTestFile).cancelCheckOut();

        STEP("Step 7: Using CMIS, verify original document has version 1.0");
        cmisAPI.usingResource(cmisTestFile).assertThat().isNotPrivateWorkingCopy()
                .then().assertThat().documentHasVersion(1.0);
        cmisAPI.usingResource(webdavTestFile).assertThat().isNotPrivateWorkingCopy()
                .then().assertThat().documentHasVersion(1.0);
        cmisAPI.usingResource(ftpTestFile).assertThat().isNotPrivateWorkingCopy()
                .then().assertThat().documentHasVersion(1.0);
        cmisAPI.usingResource(smtpTestFile).assertThat().isNotPrivateWorkingCopy()
                .then().assertThat().documentHasVersion(1.0);
    }
     ...
     * 3. U1 creates folder1 using CMIS in public site's document library. Create a file inside folder1 using SMTP
     * 4. U2 creates folder2 in public site's document library using WEBDAV
     * 5. U1 tries to copy his file to folder2 from U2 public site using FTP
     * 6. Verify file is not copied. U1 is not authorized to access the public site
     */
    @Test(groups = { TestGroup.INTEGRATION, TestGroup.REQUIRE_JMX, TestGroup.SANITY  })
    @TestRail(section = { TestGroup.INTEGRATION, TestGroup.CONTENT }, executionType = ExecutionType.SANITY,
            description = "Verify user is not able to copy file to a public site if it is not a member of that site.")
    public void copyFileToPublicSiteByUninvitedUser() throws Exception
    {
        STEP("* 1. Using CMIS create two users: u1 and u2");
        testUser1 = dataUser.createRandomTestUser();
        testUser2 = dataUser.createRandomTestUser();
        ...
        testSitePublic = dataSite.usingUser(testUser1).createPublicRandomSite();
        secondTestSitePublic = dataSite.usingUser(testUser2).createPublicRandomSite();

        STEP("* 3. U1 creates folder1 using CMIS in public site's document library. Create a file inside folder1 using SMTP");
        testFolder1 =  dataContent.usingUser(testUser1).usingSite(testSitePublic).createFolder();
        testFile1 = FileModel.getRandomFileModel(FileType.TEXT_PLAIN, "file1 content");
        ...
        testFile1.setCmisLocation(Utility.buildPath(testFolder1.getCmisLocation(), testFile1.getName()));

        STEP("* 4. U2 creates folder2 in public site's document library using WEBDAV");
        testFolder2 = FolderModel.getRandomFolderModel();
        webDavProtocol.authenticateUser(testUser2).usingSite(secondTestSitePublic).createFolder(testFolder2).assertThat().existsInWebdav();

        STEP("* 5. U1 tries to copy his file to folder2 from U2 public site using FTP"
                + "* 6. Verify file is not copied. U1 is not authorized to access the public site");
        ftpProtocol.authenticateUser(testUser1).usingResource(testFile1).copyTo(testFolder2).assertThat().doesNotExistInRepo();
    }
